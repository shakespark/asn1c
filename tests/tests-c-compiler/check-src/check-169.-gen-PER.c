 ...
 * OSS ASN.1 Tools 12.0 UPER encoder from the same values (and OSS
 ...
 * DEFAULT member explicitly set to its default value, OSS (BASIC-PER
...
check_S3_presence_and_default(void) {
	...
	/*
	 * OSS interop, decode-only: OSS (BASIC-PER encoder's choice)
	 * may encode a DEFAULT member even when it carries the default
	 * value: presence 11, a = 101 (5), b = 1, c = 00001001;
	 * 11 101 1 00001001 (14 bits) -> ec 24, as produced by the OSS
...
check_S4_untagged_choice_order(void) {
	 ...
	 * declared first. Golden bytes from OSS ASN.1 Tools 12.0.
	 */
	unsigned char buf[8];
	asn_enc_rval_t er;
	asn_dec_rval_t rv;
